...
?>>
	...
	if ($args['featured'] != 'none') {
		if ( has_post_thumbnail() && $args['featured']=='image') {
			...
			?><p class="sc_services_item_number"><?php
				printf("%02d", $number);
			?><span class="sc_services_item_subtitle"><?php
	...
	<div class="sc_services_item_info">
		...
		if (!isset($args['hide_excerpt']) || $args['hide_excerpt']==0) {
			...
			if (!empty($link)) {
				?><div class="sc_button sc_button_bordered sc_button_size_small sc_services_item_button sc_item_button"><a href="<?php echo esc_url($link); ?>" class="<?php echo esc_attr(apply_filters('trx_addons_filter_sc_item_link_classes', 'sc_services', $args)); ?>"><?php esc_html_e('Learn more', 'hoverex'); ?></a></div><?php
			}
		}
	?></div>
</div><?php
