...
if ( !function_exists('hoverex_init_meta_box') ) {
	add_action( 'after_setup_theme', 'hoverex_init_meta_box' );
	function hoverex_init_meta_box() {
		if ( is_admin() ) {
			add_action('admin_enqueue_scripts',	'hoverex_add_meta_box_scripts');
			add_action('save_post',				'hoverex_save_meta_box');
			add_action('add_meta_boxes',		'hoverex_add_meta_box');
		}
	}
}
	
// Load required styles and scripts for admin mode
if ( !function_exists( 'hoverex_add_meta_box_scripts' ) ) {
	//Handler of the add_action("admin_enqueue_scripts", 'hoverex_add_meta_box_scripts');
	function hoverex_add_meta_box_scripts() {
		// If current screen is 'Edit Page' - load font icons
		$screen = function_exists('get_current_screen') ? get_current_screen() : false;
		if (is_object($screen) && hoverex_allow_meta_box(!empty($screen->post_type) ? $screen->post_type : $screen->id)) {
...
if (!function_exists('hoverex_allow_meta_box')) {
	function hoverex_allow_meta_box($post_type) {
		return apply_filters('hoverex_filter_allow_meta_box', in_array($post_type, array('page', 'post')), $post_type);
	}
}

// Add meta box
if (!function_exists('hoverex_add_meta_box')) {
	//Handler of the add_action('add_meta_boxes', 'hoverex_add_meta_box');
	function hoverex_add_meta_box() {
		global $post_type;
		if (hoverex_allow_meta_box($post_type)) {
			add_meta_box(sprintf('hoverex_meta_box_%s', $post_type), 
						esc_html__('Theme Options', 'hoverex'),
						'hoverex_show_meta_box',
						$post_type,
...
if (!function_exists('hoverex_show_meta_box')) {
	function hoverex_show_meta_box($post=false, $args=false) {
		...
		if (hoverex_allow_meta_box($mb_post_type)) {
			// Load saved options 
			...
			if (count($tabs_titles) > 0) {
				?>
				<div class="hoverex_options hoverex_meta_box">
...
if (!function_exists('hoverex_save_meta_box')) {
	//Handler of the add_action('save_post', 'hoverex_save_meta_box');
	function hoverex_save_meta_box($post_id) {

		// verify nonce
		if ( !wp_verify_nonce( hoverex_get_value_gp('meta_box_post_nonce'), admin_url() ) )
			return $post_id;
