...
jQuery(document).ready(function() {
	...
	//-----------------------------------------------------------------------------
	jQuery('.hoverex_options .hoverex_options_section').each(function () {
	...
	function hoverex_options_check_dependencies(cont) {
		cont.find('.hoverex_options_item_field').each(function() {
			...
			if (depend) {
				...
				for (var i in depend) {
					...
					if (name.charAt(0)=='#' || name.charAt(0)=='.') {
						fld = jQuery(name);
						if (fld.length > 0 && !fld.hasClass('hoverex_inited')) {
							fld.addClass('hoverex_inited').on('change', function () {
								jQuery('.hoverex_options .hoverex_options_section').each(function () {
									hoverex_options_check_dependencies(jQuery(this));
								});
							});
