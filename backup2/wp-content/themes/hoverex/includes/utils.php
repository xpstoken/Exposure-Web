...
// $d = hoverex_array_merge($a, $b);	["four", "k2"=>"six", "seven", "k1"=>"five");
...
if (!function_exists('hoverex_get_value_gp')) {
	function hoverex_get_value_gp($name, $defa='') {
		...
		return $rez;	//hoverex_stripslashes($rez) was changed on wp_uinslash();
	}
}
	...
	function hoverex_merge_files($to, $list) {
		...
		if ( $s != '') {
			hoverex_fpc( hoverex_get_file_dir($to), 
				'/* ' 
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'hoverex') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. strpos($to, '.js')!==false 
			...
			if ($need_responsive) {
				foreach ($responsive as $k => $v) {
					if (!empty($sass['sizes'][$k])) {
						$output .= "\n\n"
								. strip_tags( sprintf( __("/* SASS Suffix: --%s */", 'hoverex'), $k) )
								. "\n"
